...
def solve_boulder_puzzle(
    ...
    lands a boulder on every switch tile. Returns None if unsolved within the
    state budget. Boulders sit on floor tiles; a push moves one boulder one
    tile if its far side is floor and unoccupied.

    ponytail: plain BFS (shortest push-path) — VR/Seafoam/Ember puzzles are
    small (≤3 boulders, ≤~90 floor tiles); upgrade to A* only if a puzzle
    blows the state budget."""
    floor, _w, _h = _walkable_grid(map_key)
    floor = floor | set(boulders)  # boulder tiles are floor underneath
    ...
    while queue:
        ...
        for d, (dx, dy) in _DIR.items():
            ...
            if nxt in bs:
                ...
                if beyond not in floor or beyond in bs:
                    continue
...
def run_boulder_puzzle(map_enum, switches, activate_boulder=None) -> Generator:
    ...
    for _resolve in range(12):
        ...
        if switch_set <= set(boulders):
            _log_event(skill="boulder_puzzle", status="solved", map=str(map_key))
            return
        start = tuple(get_player_avatar().local_coordinates)
        moves = solve_boulder_puzzle(map_key, start, boulders, switches)
        if moves is None:
            raise SkillError(f"boulder puzzle unsolvable from {start} boulders={boulders}")
        _log_event(skill="boulder_puzzle", status="executing", moves=len(moves), attempt=_resolve)
        for d in moves:
            moved = yield from _step(d)
            if not moved:
                break  # diverged — re-read live state and re-solve
            if switch_set <= set(_live_boulders(map_key)):
                _log_event(skill="boulder_puzzle", status="solved", map=str(map_key))
                return
    raise SkillError("run_boulder_puzzle: too many re-solves (stuck)")
